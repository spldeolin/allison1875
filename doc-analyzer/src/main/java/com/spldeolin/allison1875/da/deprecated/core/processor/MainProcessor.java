...
import com.spldeolin.allison1875.da.deprecated.core.definition.HandlerDefinition;
import com.spldeolin.allison1875.da.deprecated.core.enums.BodyStructureEnum;
import com.spldeolin.allison1875.da.deprecated.core.strategy.ReturnStmtBaseResponseBodyTypeParser;
import lombok.extern.log4j.Log4j2;

/**
 * 处理handler的核心主流程，处理结果可以交给视图转化器进行输出
 *
...
public class MainProcessor {
    ...
    public Collection<ApiDefinition> process() {
        ...
        new JsonPropertyDescriptionProcessor().process();

        Collection<HandlerDefinition> handlerDefinitions = new HandlerProcessor().handlerFilter(handler -> true)
                .responseBodyTypeParser(new ReturnStmtBaseResponseBodyTypeParser()).process().handlerDefinitions();

        List<ApiDefinition> result = handlerDefinitions.stream().map(handlerDefinition -> {
