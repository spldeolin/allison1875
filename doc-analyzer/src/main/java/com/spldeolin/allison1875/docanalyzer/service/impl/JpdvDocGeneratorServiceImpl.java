 ...
 */
@Singleton
public class JpdvDocGeneratorServiceImpl implements JpdvDocGeneratorService {

    @Inject
    private FieldMoreInfoAnalyzerService fieldMoreInfoAnalyzerService;

    @Override
    public String generateJpdvDoc(JsonPropertyDescriptionValueDto jpdv) {
        if (jpdv == null) {
            return null;
        }
        ...
        if (jpdv.getIsFieldCrossingValids()) {
            ...
            if (CollectionUtils.isNotEmpty(jpdv.getEcats())) {
                ...
                for (AnalyzeEnumConstantRetval ecat : jpdv.getEcats()) {
            ...
            String extra = fieldMoreInfoAnalyzerService.formatMoreInfo(jpdv.getMoreInfo());
