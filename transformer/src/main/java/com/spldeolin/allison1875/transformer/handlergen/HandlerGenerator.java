...
import com.spldeolin.allison1875.transformer.common.CuCreator;
import com.spldeolin.allison1875.transformer.handlergen.strategy.DefaultHandlerStrategy;
import com.spldeolin.allison1875.transformer.handlergen.strategy.HandlerStrategy;
...
public class HandlerGenerator {
    ...
    @Setter
    private static HandlerStrategy handlerStrategy = new DefaultHandlerStrategy();
    ...
    public static void main(String[] args) throws FileNotFoundException {
        CompilationUnit cu = StaticJavaParser.parse(new File(
                "/Users/deolin/Documents/project-repo/allison1875/transformer/src/main/java/com/spldeolin/allison1875"
                        + "/transformer/handlergen/HandlerGenerator.java"));
                ...
                for (int i = 0; i < allBlockStmt.size(); i++) {
                        ...
                        for (VariableDeclarator vd : vde.getVariables()) {
                            ...
                            if (StringUtils.equalsAny(variableName, "dto", "dtos")) {
                                vd.getInitializer().ifPresent(ir -> {
                                    ...
                                    dtoBuilder.packageName(calcDtoPackage(controllerPackage));
                    ...
                    if (dtoBuilder.typeName() == null) {
                        if (i == 0) {
                            ...
                            dtoBuilder.packageName(calcReqPackage(controllerPackage));
                            dtoBuilder.typeQualifier(dtoBuilder.packageName() + "." + dtoBuilder.typeName());
                            dtoBuilder.dtoName("req");
                            handlerBuilder.reqBodyDto(dtoBuilder);
                        } else {
                            dtoBuilder.typeName(StringUtils.upperFirstLetter(handlerBuilder.handlerName()) + "Resp");
                            dtoBuilder.packageName(calcRespPackage(controllerPackage));
                            dtoBuilder.typeQualifier(dtoBuilder.packageName() + "." + dtoBuilder.typeName());
        ...
        handlerMetaInfos.forEach(HandlerGenerator::outputHandler);
        Saves.prettySave(cu);
    }

    private static void outputHandler(ClassOrInterfaceDeclaration controller, HandlerMetaInfo metaInfo) {
    ...
    private static void outputDtos(CompilationUnit cu, Collection<DtoMetaInfo> dtos) {
    ...
    private static String standardizeVd(VariableDeclarator variable) {
    ...
    private static ClassOrInterfaceType arrayTypeToCollectionType(ArrayType arrayType) {
    ...
    private static String removeLastDollars(String text) {
        if (text == null) {
            return null;
        }
    ...
    private static String calcDtoPackage(String controllerPackage) {
        if (controllerPackage.contains(".controller")) {
            return StringUtils.replaceLast(controllerPackage, ".controller", ".javabean.dto");
        }
        return controllerPackage + ".javabean.dto";
    }

    private static String calcReqPackage(String controllerPackage) {
        if (controllerPackage.contains(".controller")) {
            return StringUtils.replaceLast(controllerPackage, ".controller", ".javabean.req");
        }
        return controllerPackage + ".javabean.req";
    }

    private static String calcRespPackage(String controllerPackage) {
        if (controllerPackage.contains(".controller")) {
            return StringUtils.replaceLast(controllerPackage, ".controller", ".javabean.resp");
        }
        return controllerPackage + ".javabean.resp";
    }
