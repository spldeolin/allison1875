...
class TypeDeclarationCollector<T extends TypeDeclaration<?>> {
    ...
    private void putAll(Collection<T> coids, Map<String, T> map) {
        for (T coid : coids) {
            try {
                Optional<String> qualifierOpt = coid.getFullyQualifiedName();
                if (!qualifierOpt.isPresent()) {
                    log.warn("Type [{}] qualifier is absent, guess it is a inner type.", coid.getNameAsString());
                    return;
                }
                String qualifier = qualifierOpt.get();
                if (map.get(qualifier) != null) {
                    // 多module的maven项目中，这样的不规范情况是可能发生的
                    log.warn("Qualifier [{}] is not unique, overwrite collected {} parsed form storage [{}].",
