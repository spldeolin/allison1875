 ...
 *
 * <p>封装了通用流程：
 * ①将 test/resources/it/persistence-generator/{caseName}/ 下的测试资源递归拷贝到 target/it/{caseName}/ 临时工作目录；
 * ②读取 .allison1875.yml 并将所有相对路径（*Module、mapperXmlDirs）解析为绝对路径后回写yml；
 * ③组装CLI参数并调用 {@link Entrypoint#main(String[])} 执行persistence-generator；
 * ④子类通过 {@link #basedir} 引用临时工作目录，在 @Test 方法中编写断言。
 *
 * <p>与 HandlerTransformerItBaseTest 的区别：
 * persistence-generator 的 DomainConfig 中有 {@code mapperXmlDirs}（相对路径列表），
 * 需要额外解析为基于 basedir 的绝对路径。
 *
 * @author Deolin 2026-05-14
 */
public abstract class PersistenceGeneratorItBaseTest {
    ...
    /**
     * 执行persistence-generator，子类在 @BeforeEach 或 @Test 中调用
     */
    protected void runPersistenceGenerator(String caseName) {
        runPersistenceGenerator(caseName, null);
    }

    /**
     * 执行persistence-generator（指定domainName），子类在 @BeforeEach 或 @Test 中调用
     */
     ...
     *
     * <p>与 HandlerTransformerItBaseTest 的区别：额外处理 DomainConfig 中的 {@code mapperXmlDirs} 字段，
