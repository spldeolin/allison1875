...
/**
 * form-generator集成测试基类。
 *
 * <p>封装了通用流程：
 * ①将 test/resources/it/form-generator/{caseName}/ 下的测试资源递归拷贝到 target/it/{caseName}/ 临时工作目录；
 * ②读取 .allison1875.yml 并将所有相对路径（*Module、mapperXmlDirs、dslPath）解析为绝对路径后回写yml；
 * ③组装CLI参数并调用 {@link Entrypoint#main(String[])} 执行form-generator；
 * ④子类通过 {@link #basedir} 引用临时工作目录，在 @Test 方法中编写断言。
 *
 * <p>form-generator 是组合工具，内部依次调用 persistence-generator、枚举生成、handler-transformer、
 * query-transformer、doc-analyzer，因此生成的产物覆盖 DDL、Entity、Mapper、Mapper XML、Controller、
 * Service、DTO、枚举等全套文件。
 *
 * @author Deolin 2026-05-17
 */
public abstract class FormGeneratorItBaseTest {
    ...
    /**
     * 执行form-generator，子类在 @Test 中调用
     */
    protected void runFormGenerator(String caseName) {
        // 1. 复制测试资源到临时工作目录
    ...
    /**
     * 将 classpath 下 it/form-generator/{caseName}/ 整个目录递归拷贝到 target/it/{caseName}/
     */
    private File copyResourceToWorkDir(String caseName) {
        String resourcePrefix = "it/form-generator/" + caseName;
    ...
    private File resolveAndRewriteConfig(File basedir) {
        ...
        // 解析 form-generator 特有的 dslPath 为绝对路径
        Object dslPathObj = configMap.get("dslPath");
        if (dslPathObj != null) {
            configMap.put("dslPath",
